...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # # print("scores")
    # # print(scores.shape)
    # (bs, num_attention_heads, seq_len, attention_head_size) = scores.shape
    # S = scores.reshape(bs, seq_len, num_attention_heads * attention_head_size)
    # print(S)
    # y = scores @ value
    y = scores.transpose(1, 2).contiguous().view(key.size(0), key.size(2), -1)
    # print(y)
    return y
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    output_dropout = dropout(dense_output)
    add_output = input + output_dropout
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    norm_output = self.add_norm(hidden_states, self_attention_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    # print(norm_output.shape)

    interm_output = self.interm_af(self.interm_dense(norm_output))
    # print(interm_output.shape)
    output = self.add_norm(norm_output, interm_output, self.out_dense, self.out_dropout, self.out_layer_norm)
    # print(output.shape)
    return output
